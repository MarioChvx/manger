...
def get_soup_page(url: str):
    """Make a request to the site and return the status code
     and a soup from the site"""
    r = requests.get(url)
...
def get_chapters_urls(soup_home: BeautifulSoup):
    ...
    chapters_urls = [a['href'] for a in soup_home.find_all('a')
...
def gen_chapters_urls(start_chapter_url: str, num_chapters: int):
    ...
    """
    chapters_urls = list()
    start_num = re.search(r'(?<=\[)(.*?)(?=\])', start_chapter_url).group(0)
    start_num = int(start_num)
    for i in range(num_chapters):
        chapter_num = start_num + i
        chapter_url = re.sub(r'\[[^]]*\]', lambda x: str(start_num + i),
                             start_chapter_url)
